...
func TestTimeSeriesContainer(t *testing.T) {
	...
	container.InsertWithUserCfg("test", 1, []float64{2.0, 0.0}, 1, MetricsTopicCpu)
	container.InsertWithUserCfg("test", 2, []float64{3.0, 0.0}, 1, MetricsTopicCpu)
	avgSigma, descOfPodTimeSeries := container.GetStatisticsOfPod("test", MetricsTopicCpu)
	assertEqual(t, avgSigma[0].Cnt(), int64(1))
	assertEqual(t, avgSigma[0].Sum(), 3.0)
	assertEqual(t, avgSigma[1].Cnt(), int64(1))
	assertEqual(t, avgSigma[1].Sum(), 0.0)
	...
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).SampleCntOfVals, int64(1))
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).MinTime, int64(2))
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).MaxTime, int64(2))

	container.ResetMetricsOfPod("test")
	container.InsertWithUserCfg("test", 3, []float64{1.0, 2.0}, 30, MetricsTopicCpu)
	container.InsertWithUserCfg("test", 4, []float64{2.0, 3.0}, 30, MetricsTopicCpu)
	container.InsertWithUserCfg("test", 5, []float64{3.0, 4.0}, 30, MetricsTopicCpu)
	avgSigma, descOfPodTimeSeries = container.GetStatisticsOfPod("test", MetricsTopicCpu)
	assertEqual(t, avgSigma[0].Cnt(), int64(3))
	assertEqual(t, avgSigma[0].Sum(), 6.0)
	assertEqual(t, avgSigma[1].Cnt(), int64(3))
	assertEqual(t, avgSigma[1].Sum(), 9.0)
	...
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).SampleCntOfVals, int64(3))
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).MinTime, int64(3))
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).MaxTime, int64(5))

	container.InsertWithUserCfg("test", 7, []float64{4.0, 5.0}, 30, MetricsTopicCpu)
	avgSigma, descOfPodTimeSeries = container.GetStatisticsOfPod("test", MetricsTopicCpu)
	assertEqual(t, avgSigma[0].Cnt(), int64(3))
	assertEqual(t, avgSigma[0].Sum(), 9.0)
	assertEqual(t, avgSigma[1].Cnt(), int64(3))
	assertEqual(t, avgSigma[1].Sum(), 12.0)
	assertEqual(t, descOfPodTimeSeries.Size, 3)
	assertEqual(t, descOfPodTimeSeries.MinTime, int64(4))
	assertEqual(t, descOfPodTimeSeries.MaxTime, int64(7))
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).SampleCntOfVals, int64(3))
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).MinTime, int64(4))
	assertEqual(t, container.GetSnapshotOfTimeSeries("test", MetricsTopicCpu).MaxTime, int64(7))

	container.InsertWithUserCfg("test", 6, []float64{5.0, 6.0}, 30, MetricsTopicCpu)
	avgSigma, descOfPodTimeSeries = container.GetStatisticsOfPod("test", MetricsTopicCpu)
	assertEqual(t, avgSigma[0].Cnt(), int64(3))
	assertEqual(t, avgSigma[0].Sum(), 9.0)
	assertEqual(t, avgSigma[1].Cnt(), int64(3))
